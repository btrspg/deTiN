...
class model:
    ...
    def calculate_TiN_likelihood(self):
        ...
        t_af_w = beta._cdf(self.afexp, self.t_alt_count + 1, self.t_ref_count + 1) - beta._cdf(self.afexp - 0.005,
                                                                                               self.t_alt_count + 1,
                                                                                               self.t_ref_count + 1)
        ...
        for seg_id, seg in self.segs.iterrows():
            ...
            TiN_likelihood[counter, :] = np.nansum(
                np.log(self.p_TiN[np.array(self.hets['seg_id'] == seg_id, dtype=bool)]),
                axis=0)
            ...
            TiN_ci_l[counter, :] = self.TiN_range[
                                       map(lambda x: x > 0.025, np.cumsum(TiN_post[counter, :])).index(True)] * 100
            TiN_ci_h[counter, :] = self.TiN_range[
                                       map(lambda x: x > 0.975, np.cumsum(TiN_post[counter, :])).index(True)] * 100
            counter += 1
    ...
    def cluster_segments(self):
        if self.n_segs >= 3:
            ...
            delta_bic = [0, 10, 20]
            self.bic = (
                               np.multiply(N, np.log(
                                   np.true_divide(np.sum(self.cl_distance_points, axis=1), N))) + np.multiply(
                           p,
                           np.log(
                               N))) + delta_bic
